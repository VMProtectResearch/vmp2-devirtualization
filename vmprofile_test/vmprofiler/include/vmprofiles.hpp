...
/// </summary>
using zydis_callback_t =
    std::function<bool(const zydis_decoded_instr_t& instr)>;
...
/// <summary>
/// pre defined vm handler profile containing all compiled time known
...
struct profile_t {
  ...
  /// <summary>
  /// size, in bits, of the operand (imm)... if there is none then this will be
  /// zero... 这个是这个vm handler会用多少字节当operand,也就是[vip] == opcode ,
  /// [vip]-[vip+imm_size]要被这个handler用到 而不是这个vm
  /// handler的操作数大小,比如addq是8字节,adddw是4字节,不是这样的
  /// </summary>
  u8 imm_size;

  /// <summary>
  /// a vector of signatures used to compare native instructions against zydis
  /// aided signatures...
  /// </summary>
...
/// <summary>
/// contains all profiles defined, as well as a vector of all of the defined
/// profiles...
/// </summary>
namespace profile {
...
extern vm::handler::profile_t lconstb2w;
...
inline std::vector<vm::handler::profile_t*> all = {
    &sregq,       &sregdw,     &sregw,       &sregb,       &lregq,
    &lregdw,      &lconstq,    &lconstbzxw,  &lconstbsxdw, &lconstbsxq,
    &lconstdwsxq, &lconstwsxq, &lconstwsxdw, &lconstdw,    &lconstw,
    &lconstb2w,   &addq,       &adddw,       &addw,        &addb,
    &popvspq,     &popvspw,    &shlq,        &shldw,       &shlw,
    &shlb,        &writeq,     &writedw,     &writew,      &writeb,
    &nandq,       &nanddw,     &nandw,       &nandb,       &shlddw,
    &shldq,       &shrq,       &shrdw,       &shrw,        &shrb,
    &shrdq,       &shrddw,     &readgsq,     &readq,       &readdw,
    &readw,       &readb,      &mulq,        &muldw,       &mulw,
    &imulq,       &imuldw,     &pushvspq,    &pushvspdw,   &pushvspw,
    &readcr8,     &readcr3,    &writecr3,    &divq,        &divdw,
    &divw,        &idivdw,     &jmp,         &lflagsq,     &vmexit,
    &call,        &rdtsc,      &cpuid};
}  // namespace profile
}  // namespace vm::handler